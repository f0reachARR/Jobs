...
報酬パイプラインの最終段（[04-reward-pipeline.md](./04-reward-pipeline.md) の段階 10）で発火する。
...
public class JobActionPaidEvent extends Event {
  ...
  public int getBaseReward();       // rare 適用後、Modifier 適用前
  public int getFinalReward();      // Modifier 適用後、Splitter 適用前
  public int getNetPaid();          // Splitter 削減後の実際の入金額
...
public class JobRewardContext {
  ...
  public int getCurrentReward();    // ここまでの段階で確定している報酬
  public boolean isRareHit();
}
...
public class JobRewardSplitContext extends JobRewardContext {
  public int getRewardAfterModifiers();
}

public class Split {
  public int deductedFromPlayer;     // プレイヤーから差し引く額

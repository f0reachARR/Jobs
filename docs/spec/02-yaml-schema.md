...
id: combat
display_name: "討伐"
description: "敵性 mob を討伐して稼ぐ。冒険と戦闘が中心。"
...
**description**：省略可。`JobConditionsDialog` や `/jobs info` で職業の位置づけを説明する 1〜3 文の短文。MiniMessage 装飾を書いても Bedrock 側では strip されるため、装飾なしでも意味が通る文にする。
...
**disclose_before_select**：`true`（default）のとき、`SpecialtyListDialog` の各職業ボタンを押しても即確定せず、まず `JobConditionsDialog` を開いて条件を提示する。詳細画面の確定ボタンで初めて `SpecialtyService#select` / `change` を呼ぶ。
`false` のときはボタン押下で即確定する（Phase 10 以前の挙動）。

**disclose_reward_amount**：`true`（default）のとき、`JobConditionsDialog` と `/jobs info` の報酬対象一覧に `RewardAmount`（`Fixed` は値、`Range` は "min〜max"）を併記する。
`false` のときは対象のみを列挙し、額は伏せる。イベント演出上、額を伏せて対象だけ知らせたい場合に使う。
rare の `chance` と `reward` はこのフラグに関わらず出さない（`announce` による初発見の演出を尊重）。

...
**RewardAmountParser**：`reward: 5` と `reward: { min: 3, max: 8 }` を吸収。
...
最後に `AdvancementProgress.revokeCriteria()` を段階 11 で呼ぶ責務は `pipeline.stage.AdvancementRevokeStage` に移す（`PipelineContext.advancementProgress` として持ち運ぶ）。
...
**RewardPipeline**：段階 1〜11 の実行器。`DetectedAction` を受け取り、`PipelineContext` を build し、`Stage` の list に沿って `execute(ctx)` を呼び続ける。
...
**stage/**：それぞれの Stage クラス（下記 11 個）。すべて `Stage#execute(PipelineContext ctx) -> StageResult` の interface に従う。
...
- `BaseRewardStage`：`RewardAmount` の解決と `amount` の掛け算。amount 解釈は spec 04 のとおり。
...
**SpecialtySelectDialog**：`MultiButtonDialog`。全ジョブを列挙。ボタンは `JobDefinition.icon`, `displayName` を使う。
ボタン押下時は Bukkit scheduler で main thread に戻し、`SpecialtyService#select` を呼ぶ（[ADR-0014](../spec/adr/0014-bedrock-dialog.md)）。

**SpecialtyChangeDialog**：変更可なら `MultiButtonDialog`、不可なら `NoticeDialog`。
現在の専業と累計獲得額を body に、選択肢を button に載せる。
...
**JobsCommands**：`Commands.literal("jobs")` のツリー構築。`select`, `change`, `status`, `reload` のサブコマンドを持つ。
`LifecycleEvents.COMMANDS` で登録（[paper/modern-commands.md](../paper/modern-commands.md)）。

**SelectSub**：`/jobs select`。専業未選択なら `SpecialtySelectDialog` を開く。既選択なら `/jobs change` を案内する。

**ChangeSub**：`/jobs change`。`SpecialtyChangeDialog` を開く。

**StatusSub**：`/jobs status`。`StatusDialog` を開く。

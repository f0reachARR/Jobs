...
- `change(playerUuid, newJobId) -> SpecialtyChangeResult`（クールダウン判定 → 成功時 insert & event）。呼び出し側でオンラインの `Player` が取れる場合、`jobs.bypass.cooldown` を持つプレイヤーはクールダウン判定をスキップする（[08-permissions.md](../spec/08-permissions.md)）。
...
コマンドツリー全体には Brigadier `.requires(...)` で `jobs.command.use` / `jobs.command.select` / `jobs.command.info` / `jobs.command.status` / `jobs.admin.reload` を張り、権限のないプレイヤーにはタブ補完から自動で隠す。

...
- `player.PlayerJob`：`playerUuid`, `jobId`, `cooldownBaseAt`。`player_job` の 1 行に対応。
- `player.PlayerJobHistory`：`id`, `playerUuid`, `jobId`, `previousJobId`, `changedAt`, `actor`, `actorUuid`。`player_job_history` の 1 行に対応。
- `player.Actor`：`PLAYER` / `ADMIN` / `SYSTEM` の 3 値 enum。
...
- `SpecialtyStage`：`SpecialtyService#currentJob(player)` を再確認。listener 時と pipeline 実行時の間に専業が変わっていた稀ケースを潰す。`jobs.bypass.specialty` を持つプレイヤーは判定をスキップして通過（[08-permissions.md](../spec/08-permissions.md)）。
- `AntiAutomationStage`：`AntiAutomationCoordinator` を呼び、0 確定なら `PipelineContext.finalReward = 0` を確定させ、以降の Stage は 0 を維持する（Stage 側で `if (ctx.zeroLocked) return CONTINUE;` チェック）。`jobs.bypass.anti-automation` を持つプレイヤーはこの Stage を丸ごとスキップ。
...
- `BuiltinModifierStage`：`VarietyPenaltyEvaluator` → `DailyCapEvaluator` の順で呼ぶ。`jobs.bypass.variety-penalty` は倍率 1.0 固定（ring buffer 更新は継続）、`jobs.bypass.daily-cap` は cap 判定をスキップ。
...
**SpecialtyService**：現在の専業取得、初回選択、変更、および管理系の強制付与・cooldown リセットを扱う。
- `currentJob(playerUuid) -> Optional<JobId>`
- `select(player, jobId) -> SpecialtyChangeResult`（初回選択、`PlayerJobRepository#upsert` と `PlayerJobHistoryRepository#append`（`actor=PLAYER`）、`JobSpecialtyChangedEvent` を発火）
- `change(player, newJobId) -> SpecialtyChangeResult`（クールダウン判定 → 成功時 upsert & history append & event）。オンラインで `jobs.bypass.cooldown` を持つプレイヤーはクールダウン判定をスキップする（[08-permissions.md](../spec/08-permissions.md)）。
- `setForced(targetUuid, jobId, actorUuid) -> SpecialtyChangeResult`（`/jobs admin set` 経路。cooldown 判定なし、`PlayerJobRepository#upsert` + `PlayerJobHistoryRepository#append`（`actor=ADMIN`）。オンラインなら caches 更新 & `JobSpecialtyChangedEvent` 発火、オフラインなら DB のみ）。
- `resetCooldown(targetUuid, actorUuid)`（`/jobs admin reset-cooldown` 経路。`PlayerJobRepository#resetCooldownBase` を呼び、オンラインなら `cooldownBaseCache` を `Instant.EPOCH` に上書き。履歴は残さない）。
- `inspect(targetUuid) -> Optional<PlayerJobRow>`（`/jobs admin inspect` 経路。オフラインでも DB から現状を返す）。
- 起動時にプレイヤーログインで `player_job` の 1 行を `currentCache` / `cooldownBaseCache` に読み込む（`MAX(selected_at)` の集計は不要）。
...
**PlayerJobRepository / PlayerJobHistoryRepository / ActionLogRepository / DailyRewardTotalRepository**：interface のみ。
シグネチャは [05-persistence.md](../spec/05-persistence.md) に従う。`PlayerJobRepository` は 1 player 1 row の現状 CRUD（`find` / `upsert` / `resetCooldownBase` / `delete`）、`PlayerJobHistoryRepository` は append-only の監査ログ（`append` / `recent` / `firstSelectedAt`）。

**dto/**：`PlayerJobRow`, `PlayerJobHistoryRow`, `Actor` enum, `ActionLogRow`, `ActionFilter`, `TimeRange`, `DailyRewardDelta`。
...
**mysql/MySql\*Repository**：`PreparedStatement` を素直に書く（jOOQ、Hibernate は使わない、[ADR-0018](../spec/adr/0018-repository-interface.md)）。`MySqlPlayerJobRepository` の `upsert` は `INSERT ... ON DUPLICATE KEY UPDATE job_id = VALUES(job_id), cooldown_base_at = VALUES(cooldown_base_at)`、`MySqlPlayerJobHistoryRepository` の `append` は素の `INSERT`。
...
**JobsCommands**：`Commands.literal("jobs")` のツリー構築。`select`, `info`, `status`, `reload`, `admin` のサブコマンドを持つ。
...
`/jobs admin <sub>` の 9 コマンド（[08-permissions.md](../spec/08-permissions.md) の Tier 1+2）。すべて `command.admin.*` のサブパッケージに 1 コマンド 1 クラスで置く。オフライン対象プレイヤーは `Bukkit.getOfflinePlayer(name)` で UUID を解決し、`PlayerJobRepository` / `PlayerJobHistoryRepository` を直接叩く（オンラインなら追加で in-memory caches も同期）。

- `AdminSetSub`：`SpecialtyService#setForced(targetUuid, jobId, senderUuid)`。
- `AdminResetCooldownSub`：`SpecialtyService#resetCooldown(targetUuid, senderUuid)`。
- `AdminInspectSub`：`SpecialtyService#inspect(targetUuid)` + `DailyTotalCache` / `DailyRewardTotalRepository` + `VarietyPenaltyEvaluator#snapshot`。オフラインは variety を「memory-only のため取得不可」とラベル付けして出す。
- `AdminPaySub`：`VaultEconomyAdapter#deposit(target, amount)` → `ActionLogWriteQueue#enqueue`（`action_key='admin:manual'`, `base=final=amount`, `rare=false`）→ `JobActionPaidEvent` 発火。パイプラインは通さない。
- `AdminResetDailyCapSub`：`DailyRewardTotalRepository#reset(uuid, today)` + `DailyTotalCache#reset(uuid)`。scope=`per_job` は次回ログインで復元される制約を help に明記する。
- `AdminResetVarietySub`：`VarietyPenaltyEvaluator#unload(uuid)`。オフライン相手は no-op で返す。
- `AdminFlushSub`：`BatchFlushWorker#flushNow(timeoutMs)`（新規 public API）。
- `AdminActionsSub`：`ActionLogRepository#recent(uuid, range, limit)`（新規メソッド）。async 実行、main thread に戻して chat 出力。
- `AdminStatsSub`：`player_job` の職業別 COUNT、`action_log` の当日 SUM、rare 発火率を集計。async。

コマンドツリー全体には Brigadier `.requires(...)` で `jobs.command.use` / `jobs.command.select` / `jobs.command.info` / `jobs.command.status` を張り、`/jobs admin *` の各枝には対応する `jobs.admin.<sub>` を張る。権限のないプレイヤーにはタブ補完から自動で隠される。

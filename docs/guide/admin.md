...
かまどの精錬元帳は KVS ではなくブロックの PDC に持つ別系統のデータだが、`kvs block` の出力には一緒に出る（[../spec/05-persistence.md](../spec/05-persistence.md) の「精錬元帳（Block PDC）」）。TTL は無く、チャンクと一緒に保存される。旧バージョンが書いた `op:furnace:*` のエントリが残っていても、精錬の判定には使われない。

| サブコマンド | 動作 |
|---|---|
| `kvs block [<x> <y> <z>]` | 視線の先、または指定座標のブロックの追跡データを一覧する。かまどなら精錬元帳（投入者と個数）も出す。座標を省略した場合はコンソールから実行できない |
...
3. かまどを見ていれば「精錬元帳」の行が出る。問い合わせてきたプレイヤーのエントリが無い、または「投入者なし」になっていれば、その材料は本人の投入として記録されていない（= 精錬報酬は出ない）。
4. 誤検知と判断したら `/jobs admin kvs remove <key>` で消す。TTL 切れを待つ必要はない。精錬元帳を消す口は用意していない（材料を取り出して入れ直せば書き直される）。
...
5. 精錬（`item_smelted`）だけ入らない場合は、そもそも報酬イベントが起きていない可能性が高い。報酬は取り出した人ではなく投入した人に、焼き上がった時点で入る。かまどを見ながら `/jobs admin kvs block` を実行し、精錬元帳に本人のエントリがあるかを確認する（ホッパー投入ぶんと、焼き上がった瞬間にオフラインだったぶんは払われない）。
